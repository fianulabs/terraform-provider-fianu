...
// those are populated by the server's CEL compiler when the gate is
...
//   - indexes (no asset)    → references one or more existing fianu_index entities
//   - asset only (no exprs) → unscoped variation; server links the default
//     index for that asset type
//
// Mirrors `internal/resources/policy/criteria.go`'s criteriaModel — gates and
// policies share the PolicyAssetGroup wire shape via the GateProtectedScope
// embed, so the HCL surface stays symmetric.
...
func criteriaAttribute() schema.SingleNestedAttribute {
	return schema.SingleNestedAttribute{
		MarkdownDescription: "Asset group criteria. Restricts this variation to assets matching either a set of CEL expressions or one or more existing indexes. When omitted, the variation applies to every asset in the gate's scope.",
		Optional:            true,
...
func (c *criteriaModel) toEntity() *fianu_entities.PolicyAssetGroup {
	...
	if len(c.Expressions) > 0 {
		...
		for i, e := range c.Expressions {
			...
			// Pre-parse via cel.ParseExpression so ExprSource carries the
			// canonical form the validator expects. See
